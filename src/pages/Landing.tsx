...
const MOCK_PROS = [
  { name: 'Priya Sharma', role: 'SDE-2', company: 'Google', rating: 4.9, referrals: 47, gradient: 'from-[#3B5FE5] to-[#8B8FD4]' },
    ...
    <div className="flex flex-col gap-4 px-2 sm:px-4 py-4 sm:py-8">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: active ? 1 : 0, y: active ? 0 : 8 }} transition={{ delay: 0.15 }} className="rounded-xl border border-border/40 bg-card p-4 space-y-3">
        <div className="flex items-center gap-2">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-[#3B5FE5] to-[#8B8FD4] text-white text-[10px] font-bold">PS</div>
          <div>
...
function GetReferredScene({ active }: { active: boolean }) {
  return (
    <div className="flex flex-col items-center gap-5 px-4 py-8 sm:py-12">
      <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: active ? 1 : 0, scale: active ? 1 : 0.8 }} transition={{ delay: 0.2, type: 'spring', stiffness: 100 }} className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-secondary shadow-glow-lg">
        <span className="font-display text-2xl font-extrabold text-white">DR</span>
      </motion.div>
      ...
      <div className="overflow-hidden rounded-xl sm:rounded-2xl border border-border/60 bg-card shadow-2xl">
        ...
        <div className="flex items-center justify-between border-b border-border/30 bg-muted/10 px-3 sm:px-4 py-2">
          ...
          <div className="flex items-center gap-1">
            {SCENES.map((_, i) => (
              <button key={i} onClick={() => setIdx(i)} className={`h-1.5 rounded-full transition-all duration-300 ${i === idx ? 'w-6 bg-primary' : i < idx ? 'w-1.5 bg-primary/40' : 'w-1.5 bg-muted-foreground/20'}`} aria-label={`Go to scene ${i + 1}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between border-t border-border/30 bg-muted/10 px-3 sm:px-4 py-2.5">
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground" onClick={prev} disabled={idx === 0}><SkipBack className="h-3.5 w-3.5" /></Button>
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground" onClick={() => setPlaying(p => !p)}>{playing ? <Pause className="h-3.5 w-3.5" /> : <Play className="h-3.5 w-3.5" />}</Button>
          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground" onClick={next} disabled={idx === SCENE_COUNT - 1}><SkipForward className="h-3.5 w-3.5" /></Button>
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-4">
          <div className="text-center">
            <div className="font-display text-2xl font-bold text-[#4F7CFF]">12</div>
            <div className="text-[10px] text-[#9CA3AF]">Referrals sent</div>
          </div>
          <div className="h-10 w-px bg-[rgba(255,255,255,0.06)]" />
          <div className="text-center">
            <div className="font-display text-2xl font-bold text-[#4ADE80]">78</div>
            <div className="text-[10px] text-[#9CA3AF]">Profile score</div>
          </div>
          <div className="h-10 w-px bg-[rgba(255,255,255,0.06)]" />
          <div className="text-center">
            <div className="font-display text-2xl font-bold text-[#FBBF24]">3</div>
            <div className="text-[10px] text-[#9CA3AF]">Interviews</div>
          </div>
        </div>
      </div>
      <div className="flex flex-wrap gap-2">
        <Badge className="border border-[rgba(74,222,128,0.2)] bg-[rgba(74,222,128,0.1)] text-[#4ADE80] text-[10px]">Accepted {'\u2022'} Flipkart</Badge>
        <Badge className="border border-[rgba(79,124,255,0.2)] bg-[rgba(79,124,255,0.1)] text-[#4F7CFF] text-[10px]">Applied {'\u2022'} Razorpay</Badge>
      ...
      <div className="rounded-xl bg-[rgba(255,255,255,0.03)] p-4">
        <div className="text-xs font-medium text-[#9CA3AF] mb-3">Pipeline</div>
        <div className="flex gap-3">
          {['Request Sent', 'Under Review', 'Accepted', 'Submitted', 'Hired'].map((step, i) => (
            <div key={step} className="flex-1 text-center">
              <div className={`h-1.5 rounded-full mb-1.5 ${i <= 2 ? 'bg-[#4F7CFF]' : 'bg-[rgba(255,255,255,0.06)]'}`} />
              <span className={`text-[9px] ${i <= 2 ? 'text-[#4F7CFF]' : 'text-[#6B7280]'}`}>{step}</span>
            </div>
          ))}
        </div>
      </div>
...
function ProfessionalsTab() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 rounded-lg bg-[rgba(255,255,255,0.04)] px-3 py-2">
        <Search className="h-3.5 w-3.5 text-[#6B7280]" />
        <span className="text-xs text-[#6B7280]">Search by company, skill, or role...</span>
      </div>
      {pros.map(p => (
        <div key={p.name} className="flex items-center gap-3 rounded-xl bg-[rgba(255,255,255,0.03)] p-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 text-xs font-semibold text-white">{p.name} <ShieldCheck className="h-3 w-3 text-[#4ADE80]" /></div>
            <div className="text-[11px] text-[#9CA3AF]">{p.role} {'\u2022'} {p.company}</div>
          </div>
          ...
          <Button size="sm" variant="outline" className="shrink-0 rounded-full border-[rgba(255,255,255,0.06)] text-[10px] text-white hover:bg-[rgba(255,255,255,0.04)]">Request</Button>
        </div>
      ))}
    </div>
  )
...
function ReferralsTab() {
  const jobs = [
    { company: 'Flipkart', role: 'SDE-2', stage: 'Accepted', color: '#4ADE80', date: '2 days ago' },
    { company: 'Razorpay', role: 'Frontend Engineer', stage: 'Applied', color: '#4F7CFF', date: '5 days ago' },
    { company: 'Google', role: 'L3 SWE', stage: 'Under Review', color: '#FBBF24', date: '1 week ago' },
  ]
  return (
    <div className="space-y-3">
      {jobs.map(j => (
        <div key={j.company + j.role} className="flex items-center gap-3 rounded-xl bg-[rgba(255,255,255,0.03)] p-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[rgba(255,255,255,0.05)]"><Briefcase className="h-4.5 w-4.5 text-[#9CA3AF]" /></div>
          <div className="flex-1 min-w-0">
            <div className="text-xs font-semibold text-white">{j.role}</div>
            <div className="text-[11px] text-[#9CA3AF]">{j.company}</div>
          </div>
          <div className="text-right">
            <div className="text-[9px] font-medium" style={{ color: j.color }}>{j.stage}</div>
            <div className="mt-0.5 text-[9px] text-[#6B7280]">{j.date}</div>
          </div>
        </div>
      ))}
      <div className="rounded-xl bg-[rgba(255,255,255,0.03)] p-3">
        <div className="text-[10px] font-medium text-[#9CA3AF] mb-2">Pipeline Progress</div>
        <div className="flex items-center gap-1">
          {['Sent', 'Review', 'Accepted', 'Submitted', 'Hired'].map((step, i) => (
            <div key={step} className="flex-1">
              <div className={`h-1 rounded-full ${i < 2 ? 'bg-[#4ADE80]' : 'bg-[rgba(255,255,255,0.06)]'}`} />
              <span className={`text-[8px] mt-1 block ${i < 2 ? 'text-[#4ADE80]' : 'text-[#6B7280]'}`}>{step}</span>
            </div>
          ))}
        </div>
      </div>
...
function InteractiveMock() {
  ...
  return (
    <div className="hero-mock-anim relative mx-auto mt-12 max-w-4xl px-0 sm:px-4">
      ...
      <div className="overflow-hidden rounded-xl sm:rounded-2xl border border-[rgba(255,255,255,0.06)] bg-[#151A21] shadow-2xl">
        <div className="flex items-center gap-2 border-b border-[rgba(255,255,255,0.04)] bg-[rgba(255,255,255,0.02)] px-3 sm:px-4 py-2 sm:py-2.5">
          <span className="h-2.5 w-2.5 rounded-full bg-rose-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-amber-400" />
          <span className="h-2.5 w-2.5 rounded-full bg-emerald-400" />
        ...
        <div className="flex border-b border-[rgba(255,255,255,0.04)] bg-[rgba(255,255,255,0.01)] px-2 gap-1">
          {MOCK_TABS.map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`flex items-center gap-1.5 rounded-t-lg px-3 py-2 text-[11px] font-medium transition-all ${activeTab === tab.id ? 'bg-[rgba(255,255,255,0.04)] text-white border-t border-x border-[rgba(255,255,255,0.06)]' : 'text-[#6B7280] hover:text-[#9CA3AF]'}`}>
...
function WaitlistForm({ className = '' }: { className?: string }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed) { setStatus('error'); setMessage('Please enter your email address.'); return }
    if (!isValidEmail(trimmed)) { setStatus('error'); setMessage('Please enter a valid email address.'); return }
    setStatus('loading')
    try {
      const res = await (await fetch('/api/waitlist', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: trimmed, name: '', company_slug: 'general', company_name: 'General Waitlist' }),
      })).json()
      if (res.success) {
        setStatus('success')
        setMessage(res.data?.message || "You're on the list! We'll notify you when we launch.")
      } else {
        setStatus('error')
  ...
  if (status === 'success') {
    return (
      <div className={`rounded-2xl border border-green-500/20 bg-green-500/5 p-8 text-center space-y-4 ${className}`}>
        <CircleCheckBig className="h-12 w-12 text-green-500 mx-auto" />
        <h3 className="text-xl font-bold text-foreground">{"You're on the list!"}</h3>
        <p className="text-sm text-muted-foreground max-w-md mx-auto">{message} Check your inbox for a confirmation email.</p>
      </div>
    )
  }

  return (
    <div className={`rounded-2xl border border-border/60 bg-card p-6 sm:p-8 space-y-5 ${className}`}>
      <div className="text-center space-y-2">
        <div className="flex items-center justify-center gap-2">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10"><Bell className="h-5 w-5 text-primary" /></div>
        </div>
        <h3 className="text-lg font-bold text-foreground">Get early access</h3>
        <p className="text-sm text-muted-foreground max-w-md mx-auto">Join the waitlist and be the first to know when Direct Refer launches. No spam &mdash; we only email you once.</p>
      </div>
      {user ? (
        <div className="text-center">
          <Button size="lg" className="rounded-full bg-primary px-6 shadow-glow" asChild>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto">
          <input
            ...
            className="flex-1 rounded-xl border border-border bg-background px-4 py-3 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary"
            aria-label="Email address"
          />
          <Button type="submit" disabled={status === 'loading'} className="rounded-full bg-primary px-6 py-3 shadow-glow hover:shadow-[0_4px_30px_-4px_rgba(79,124,255,0.5)] transition-all duration-300">
            {status === 'loading' ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Joining...</> : <><Bell className="mr-2 h-4 w-4" /> Join Waitlist</>}
          </Button>
        </form>
      )}
      {status === 'error' && <p className="text-sm text-red-500 text-center">{message}</p>}
      <p className="text-xs text-muted-foreground/60 text-center">No spam. One email when we launch. Unsubscribe anytime.</p>
    </div>
  )
}
      ...
      <main id="main-content">
        ...
        <section className="relative overflow-hidden px-4 pb-14 pt-10 sm:px-6 sm:pb-24 sm:pt-20 lg:px-8">
          ...
          <div className="mx-auto max-w-3xl text-center relative z-10">
            ...
            <h1 className="hero-anim-2 font-display mt-6 text-2xl font-extrabold leading-[1.08] tracking-tight sm:text-4xl lg:text-6xl">
            ...
            <div className="hero-anim-4 mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Button size="lg" className="rounded-full bg-primary px-6 sm:px-8 shadow-glow hover:shadow-[0_4px_30px_-4px_rgba(79,124,255,0.5)] transition-all duration-300 text-sm sm:text-base" asChild>
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="mt-12 grid gap-8 sm:grid-cols-3">
              {[
                ...
                <FadeIn key={s.step} delay={i * 0.12} className="text-center group">
                  <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-primary to-secondary text-white shadow-glow-lg group-hover:scale-110 transition-transform duration-300" style={{ animationDelay: `${i * 0.5}s` }}>
            ...
            <div className="mt-12 grid gap-5 sm:grid-cols-3">
              {[
                ...
                <FadeIn key={item.title} delay={i * 0.1}>
                  <SpotlightCard className="h-full rounded-2xl border border-border/60 bg-card p-5 sm:p-6 space-y-4">
                      ...
                      <ul className="mt-4 space-y-2.5">
                        {r.points.map(p => (
                          <li key={p} className="flex items-center gap-2 text-sm">
                            <CheckCircle2 className="h-4 w-4 text-[#4ADE80] shrink-0" /> {p}
                          </li>
                        ))}
                      </ul>
                    </CardContent>
        ...
        <section className="relative overflow-hidden border-t border-border/50 px-4 py-12 text-center sm:px-6 sm:py-20 lg:px-8">
          ...
          <FadeIn>
            <h2 className="font-display text-2xl sm:text-3xl lg:text-5xl font-bold tracking-tight text-foreground">
        ...
        <div className="mx-auto max-w-5xl">
          <div className="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="flex flex-col items-center gap-2.5 sm:items-start">
              ...
              <Link to="/blog" className="text-xs text-muted-foreground hover:text-foreground transition-colors py-0.5">Blog</Link>
              <Link to="/changelog" className="text-xs text-muted-foreground hover:text-foreground transition-colors py-0.5">Changelog</Link>
