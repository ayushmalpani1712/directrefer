...
import { CompanyChip, DashboardSkeleton, EmptyState, GAvatar, ProgressRing, SectionHeader, StatCard, StatusBadge } from '@/components/ui-kit'
...
const QUICK_ACTIONS = [
  { icon: Send, label: 'Apply for Referral', sublabel: 'Get referred to your dream role', href: '/professionals' },
  { icon: FileUp, label: 'Upload Resume', sublabel: 'Keep your profile updated', href: '/profile' },
  { icon: Target, label: 'Improve Profile', sublabel: 'Increase your chances', href: '/profile' },
  { icon: GraduationCap, label: 'Learning', sublabel: 'Resources & guides', href: '/help' },
]

export default function StudentDashboard() {
  const loading = usePageLoading()
  const { visibleProfessionals: professionals, student, bookmarks, referralsSentToday, requests } = useApp()
  ...
  const myRequests = requests.filter((r) => r.student === student.name)
  const saved = professionals.filter((p) => bookmarks.includes(p.id))
  const recommended = professionals.filter((p) => p.openForReferrals).slice(0, 3)
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <h1 className="font-display mt-0.5 text-[28px] font-bold leading-tight text-foreground">
                {student.name.split(' ')[0]}, your referral pipeline is {myRequests.filter((r) => r.status === 'pending').length > 0 ? 'active' : 'ready to go'}
              </h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 items-stretch">
        <StatCard icon={Send} label="Referrals sent" value={myRequests.length} delta={myRequests.length > 0 ? Math.round((myRequests.length / Math.max(1, myRequests.length + 2)) * 100) : 0} deltaLabel="completion" delay={0.05} href="/applications" />
        <StatCard icon={TrendingUp} label="Acceptance rate" value={myRequests.length > 0 ? `${Math.round((myRequests.filter((r) => r.status === 'accepted' || r.status === 'offered').length / myRequests.length) * 100)}%` : '0%'} delta={myRequests.filter((r) => r.status === 'accepted' || r.status === 'offered').length} deltaLabel="accepted" delay={0.1} href="/applications" />
        <StatCard icon={Users} label="Active referrals" value={myRequests.filter((r) => r.status === 'pending').length} delta={myRequests.filter((r) => r.status === 'pending').length} deltaLabel="awaiting reply" delay={0.15} href="/applications" />
      </div>
        ...
        <div className="flex flex-col gap-5 lg:col-span-2">
          ...
          <Link to="/applications" className="block">
            <Card className="shadow-soft cursor-pointer transition-all duration-200 hover:border-primary/15">
              ...
              <CardContent className="pt-0">
                {myRequests.length === 0 ? (
                  <EmptyState icon={Send} title="No referrals yet" description="Browse verified professionals and send your first request." action={<Button asChild><Link to="/professionals">Find professionals</Link></Button>} />
                ) : (
          ...
          <div className="flex flex-1 flex-col">
            <SectionHeader
              ...
              subtitle="Matched to your skills and target companies"
              action={<Button variant="ghost" size="sm" className="h-7 text-xs text-primary hover:text-primary/70" asChild><Link to="/professionals">Browse all <ArrowRight className="ml-1 h-3 w-3" /></Link></Button>}
            />
            <div className="grid flex-1 gap-4 sm:grid-cols-3">
              {recommended.map((p) => (
                <div key={p.id} className="h-full">
                  <Link to={`/professionals/${p.id}`} className="block h-full">
                    <Card className="group h-full shadow-soft transition-all duration-200 hover:border-primary/15 hover:bg-card">
                      <CardContent className="flex h-full flex-col p-5 text-center">
                        ...
                        <div className="mt-auto pt-4">
                          ...
                          <Button size="sm" className="w-full rounded-lg bg-gradient-to-r from-[#4F7CFF] to-[#7C5CFF] text-white hover:opacity-90" asChild>
                            <Link to={`/request-referral/${p.id}`}><Send className="mr-1.5 h-3.5 w-3.5" /> Request referral</Link>
                          </Button>
                        </div>
                      </CardContent>
                    </Card>
              ...
              <CardContent className="flex-1 pt-0">
                <ScrollArea className="h-[200px]">
                  <div className="space-y-3">
                    {useApp().activity.slice(0, 5).map((a) => (
