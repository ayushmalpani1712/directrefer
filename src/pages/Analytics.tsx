...
const TICK = { fill: '#94a3b8', fontSize: 12 } as const
...
function StudentAnalytics({ range }: { range: DateRange }) {
  ...
  const [conversion, setConversion] = useState<{ name: string; value: number; fill: string }[]>([
    ...
    { name: 'Accepted', value: 0, fill: 'hsl(152 69% 40%)' },
    { name: 'In Review', value: 0, fill: 'hsl(243 75% 59%)' },
    { name: 'Declined', value: 0, fill: 'hsl(0 72% 55%)' },
  ])
  ...
  const fetchStudentData = useCallback(async () => {
    try {
      ...
      setConversion([
        ...
        { name: 'In Review', value: inReview, fill: 'hsl(243 75% 59%)' },
  ...
  const pipelineBarColors = ['hsl(243 75% 59%)', 'hsl(263 70% 58%)', 'hsl(152 69% 40%)', 'hsl(320 70% 60%)', 'hsl(38 92% 50%)']
          ...
          {hasData(studentWeekly) ? (
            <LazyResponsiveContainer width="100%" height={280}>
              <LazyBarChart data={studentWeekly} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                ...
                <LazyLegend wrapperStyle={{ fontSize: 12, color: '#94a3b8' }} />
                <LazyBar dataKey="applications" stackId="a" fill="hsl(243 75% 59%)" radius={[0, 0, 0, 0]} name="Applications" />
                <LazyBar dataKey="referrals" stackId="a" fill="hsl(263 70% 58%)" name="Referrals" />
                <LazyBar dataKey="responses" stackId="a" fill="hsl(320 70% 60%)" radius={[4, 4, 0, 0]} name="Responses" />
              </LazyBarChart>
        ...
        {hasData(studentWeekly) ? (
          <LazyResponsiveContainer width="100%" height={240}>
            <LazyLineChart data={studentWeekly.map((w) => ({ month: w.label, rate: Math.round((w.responses / Math.max(w.applications, 1)) * 100) }))} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
              ...
              <LazyTooltip content={<ChartTooltip />} />
              <LazyLine type="monotone" dataKey="rate" stroke="hsl(152 69% 40%)" strokeWidth={2.5} dot={{ r: 3 }} name="Success rate" />
            </LazyLineChart>
          </LazyResponsiveContainer>
        ) : <EmptyChart />}
      </ChartCard>
          ...
          {hasData(proMonthly) ? (
            <LazyResponsiveContainer width="100%" height={260}>
              <LazyAreaChart data={proMonthly} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                ...
                <LazyTooltip content={<ChartTooltip />} />
                <LazyLegend wrapperStyle={{ fontSize: 12, color: '#94a3b8' }} />
                <LazyArea type="monotone" dataKey="referrals" stroke="hsl(243 75% 59%)" strokeWidth={2.5} fill="url(#a1)" name="Received" />
                <LazyArea type="monotone" dataKey="accepted" stroke="hsl(152 69% 40%)" strokeWidth={2.5} fill="transparent" name="Accepted" />
              </LazyAreaChart>
            </LazyResponsiveContainer>
          ) : <EmptyChart />}
        </ChartCard>
            ...
            <LazyResponsiveContainer width="100%" height={260}>
              <LazyBarChart data={proResponseTime} margin={{ top: 8, right: 8, left: -24, bottom: 0 }}>
                ...
                <LazyBar dataKey="hours" radius={[5, 5, 0, 0]} name="Hours to reply">
                  {proResponseTime.map((d, i) => <LazyCell key={d.label} fill={i === 4 ? 'hsl(152 69% 40%)' : 'hsl(243 75% 59%)'} />)}
                </LazyBar>
              </LazyBarChart>
            </LazyResponsiveContainer>
        </ChartCard>
          ...
          <LazyResponsiveContainer width="100%" height={250}>
            <LazyFunnelChart>
              ...
              <LazyFunnel dataKey="value" data={funnelData} isAnimationActive>
                ...
                <LazyLabelList position="right" fill="#94a3b8" stroke="none" dataKey="stage" fontSize={11} />
              </LazyFunnel>
            </LazyFunnelChart>
          </LazyResponsiveContainer>
        </ChartCard>
          ...
          {hasData(recruiterWeekly) ? (
            <LazyResponsiveContainer width="100%" height={250}>
              <LazyAreaChart data={recruiterWeekly} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
                ...
                <LazyTooltip content={<ChartTooltip />} />
                <LazyLegend wrapperStyle={{ fontSize: 12, color: '#94a3b8' }} />
                <LazyArea type="monotone" dataKey="applications" stroke="hsl(243 75% 59%)" strokeWidth={2.5} fill="url(#r1)" name="Applications" />
                <LazyArea type="monotone" dataKey="hires" stroke="hsl(152 69% 40%)" strokeWidth={2.5} fill="transparent" name="Hires" />
              </LazyAreaChart>
            </LazyResponsiveContainer>
          ) : <EmptyChart />}
        </ChartCardGrid>
