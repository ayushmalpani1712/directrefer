...
export default function JobDetailPage() {
  ...
  const toggleBookmark = async () => {
    ...
    try {
      if (bookmarked) {
        await supabase.from('bookmarks').delete().eq('user_id', user.id).eq('professional_id', id)
        setBookmarked(false)
        toast.success('Bookmark removed')
      } else {
        await supabase.from('bookmarks').insert({ user_id: user.id, professional_id: id })
        setBookmarked(true)
  ...
  if (isMobile) {
    return (
      <div className="pb-[140px]">
        ...
        <div className="px-4 space-y-5">
          ...
          {!job.description && !job.requirements && (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">Contact the recruiter for more info.</p>
            </div>
          )}
        ...
        <div className="fixed bottom-[72px] left-0 right-0 z-40 border-t bg-background/95 backdrop-blur-lg safe-area-inset-bottom">
