...
import { Chip, CompanyChip, DashboardSkeleton, GAvatar, StatCard } from '@/components/ui-kit'
...
export default function RecruiterDashboard() {
  ...
  const { jobs, candidates, savedCandidates, student, activity } = useApp()
  const { user } = useAuth()
  const [recruiterCompany, setRecruiterCompany] = useState({
    name: 'Acme Corp', industry: 'Technology', size: '500-1000',
    website: 'acme.com', description: 'A leading technology company.',
    hiringStats: { timeToHire: 21, offerAccept: 78, referralShare: 34, activeJobs: jobs.filter((j) => j.stage === 'Active').length },
    responseRate: 92, verified: true,
  })
  const RECRUITER_USER = { name: student.name, designation: '', gradient: student.gradient }
  ...
  const [funnelData, setFunnelData] = useState<FunnelStage[]>([
    { stage: 'Applied', value: 0, fill: 'hsl(243 75% 59%)' },
    { stage: 'Screened', value: 0, fill: 'hsl(263 70% 58%)' },
    { stage: 'Review', value: 0, fill: 'hsl(290 65% 58%)' },
    { stage: 'Offer', value: 0, fill: 'hsl(320 70% 60%)' },
    { stage: 'Hired', value: 0, fill: 'hsl(152 69% 40%)' },
  ])
  ...
  const fetchFunnel = useCallback(async () => {
    ...
    setFunnelData([
      { stage: 'Applied', value: counts.Applied, fill: 'hsl(243 75% 59%)' },
      { stage: 'Screened', value: counts.Screened, fill: 'hsl(263 70% 58%)' },
      { stage: 'Review', value: counts.Review, fill: 'hsl(290 65% 58%)' },
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="relative p-4 sm:p-5">
            ...
            <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                <Button className="bg-gradient-to-r from-[#4F7CFF] to-[#7C5CFF] text-white shadow-sm hover:translate-y-[-1px]" asChild><Link to="/jobs"><Plus className="mr-1.5 h-4 w-4" /> Post a job</Link></Button>
                <Button variant="outline" className="border-border bg-transparent text-foreground hover:bg-muted/30" asChild><Link to="/talent"><Search className="mr-1.5 h-4 w-4" /> Search talent</Link></Button>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4 items-stretch">
        <StatCard icon={Briefcase} label="Open jobs" value={jobs.filter((j) => j.stage === 'Active').length} delta={0} delay={0.05} href="/jobs" />
        <StatCard icon={Users} label="Total applicants" value={totalApplicants} delta={0} delay={0.1} href="/jobs" />
        <StatCard icon={UserCheck} label="In review" value={inReview} delta={0} delay={0.15} href="/talent" />
        ...
        <div className="flex flex-col gap-6 lg:col-span-2">
                ...
                <LazyResponsiveContainer width="100%" height={230}>
                  <LazyFunnelChart>
                    ...
                    <LazyFunnel dataKey="value" data={funnelData} isAnimationActive>
                      ...
                      <LazyLabelList position="right" fill="#9CA3AF" stroke="none" dataKey="stage" fontSize={11} />
                    </LazyFunnel>
                  </LazyFunnelChart>
              ...
              <CardContent className="pt-2">
                ...
                <LazyResponsiveContainer width="100%" height={220}>
                  <LazyBarChart data={recruiterWeekly} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                    ...
                    <LazyBar dataKey="applications" radius={[4, 4, 0, 0]} fill="#4F7CFF" name="Applications" />
          ...
          <Card className="shadow-soft cursor-pointer transition-all duration-200 hover:border-primary/15">
            <CardHeader className="">
              ...
              <span data-slot="card-action" className="shrink-0"><Button variant="ghost" size="sm" className="text-primary">Manage all <ArrowRight className="ml-1 h-3.5 w-3.5" /></Button></span>
            </CardHeader>
            <CardContent className="space-y-3 pt-2">
              {jobs.filter((j) => j.stage === 'Active').slice(0, 4).map((j) => (
                <Link to="/jobs" key={j.id} className="flex items-center gap-4 rounded-xl border border-border bg-card p-4 transition-colors hover:border-primary/20">
        ...
        <div className="flex flex-col gap-6">
          ...
          <Card className="shadow-soft cursor-pointer transition-all duration-200 hover:border-primary/15">
            <CardHeader className="">
              ...
              <span data-slot="card-action" className="shrink-0"><Button variant="ghost" size="sm" className="text-primary">All</Button></span>
            </CardHeader>
            <CardContent className="space-y-2.5 pt-2">
              {saved.map((c) => (
                <Link to="/talent" key={c.id} className="flex items-center gap-3 rounded-lg p-1.5 transition-colors hover:bg-muted/20">
