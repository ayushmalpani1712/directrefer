...
interface AuthState {
  ...
  signInWithGoogleIdToken: (idToken: string, nonce?: string) => Promise<{ error?: string }>
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = useCallback(async (email: string, password: string, meta?: { full_name?: string; role?: string }) => {
    ...
    // Ensure user row exists (fire-and-forget — don't block login)
    if (data.user) {
      ensureUserRow(data.user).catch(() => {})
    }
  ...
  const value = useMemo<AuthState>(() => ({
    user, session, loading, emailVerified, needsVerification, professionalVerified, recruiterVerified, activeWorkspace, signUp, signIn, signInWithGoogle, signInWithGoogleIdToken, signInWithLinkedIn, signOut, refreshWorkspaceStatus,
  }), [user, session, loading, emailVerified, needsVerification, professionalVerified, recruiterVerified, activeWorkspace, signUp, signIn, signInWithGoogle, signInWithGoogleIdToken, signInWithLinkedIn, signOut, refreshWorkspaceStatus])
